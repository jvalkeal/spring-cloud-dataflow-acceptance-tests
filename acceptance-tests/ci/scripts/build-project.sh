...
./gradlew clean build -PartifactoryDockerPush=${ARTIFACTORY_DOCKER_PUSH} -PartifactoryDockerLocal=${ARTIFACTORY_DOCKER_LOCAL} -PdataflowIncludeTags="${DATAFLOW_INCLUDE_TAGS}" -PdataflowExcludeTags="${DATAFLOW_EXCLUDE_TAGS}" || n=1
